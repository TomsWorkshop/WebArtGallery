...
public class LeftDoorController : MonoBehaviour
{
    ...
    {
        //  float speed = (targetAngle - (LeftJoint.transform.localEulerAngles.y - initialLeftJointRotate.y)) * 0.01f;    // P制御
        float speed = System.Math.Abs((targetAngle + 180 - (LeftJoint.transform.localEulerAngles.y - initialLeftJointRotate.y))) * 0.01f;    // P制御
    ...
    {
        float speed = (targetPosition - (LeftJoint.transform.localPosition.x - initialLeftJointPosition.x)) * 0.01f;  // P制御
